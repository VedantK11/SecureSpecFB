...
uint32_t O3_CPU::add_to_rob(ooo_model_instr *arch_instr)
{
    ...
    if(SHADOW_BUFFER.tail == 0)
        shadow_index = SHADOW_BUFFER.SIZE - 1;
    else
        shadow_index = SHADOW_BUFFER.tail - 1;
...
uint32_t O3_CPU::add_to_shadow_buffer(ooo_model_instr *arch_instr)
{
    ...
    if(SHADOW_BUFFER.entry_pointer[index] != NULL)
    {
        ...
        {
            cerr << "[SHADOW_BUFFER_ERROR] " << __func__ << " is not empty index: " << index;
            cerr << " instr_id: " << SHADOW_BUFFER.entry_pointer[index]->instr_id << endl;
                assert(0);
        }        
    } 
    SHADOW_BUFFER.entry_pointer[index] = arch_instr;
    ...
    if(arch_instr->instr_id == DEBUG_INSTRUCTION)
    {
        ...
        // cout << " head: " << SHADOW_BUFFER.head << " tail: " << SHADOW_BUFFER.tail << " occupancy: " << SHADOW_BUFFER.occupancy;
        // cout << " event: " << SHADOW_BUFFER.entry[index].event_cycle << " current: " << current_core_cycle[cpu] << endl; });

#ifdef SANITY_CHECK
    if (SHADOW_BUFFER.entry_pointer[index]->ip == 0)
    {
        cerr << "[SHADOW_BUFFER_ERROR] " << __func__ << " ip is zero index: " << index;
        cerr << " instr_id: " << SHADOW_BUFFER.entry_pointer[index]->instr_id << " ip: " << SHADOW_BUFFER.entry_pointer[index]->ip << endl;
        assert(0);
    }
#endif
    return index;
	  ...
	  for(uint32_t i=0; i<DECODE_BUFFER.SIZE; i++)
	  {
			...
			if(DECODE_BUFFER.entry[DECODE_BUFFER.head].btb_miss == 1 && DECODE_BUFFER.entry[DECODE_BUFFER.head].branch_mispredicted == 0)
		{
            ...
            if (ROB.entry[rob_index].is_branch /*|| ROB.entry[rob_index].is_memory*/)
                uint32_t shadow_buffer_index = add_to_shadow_buffer(&ROB.entry[rob_index]);
