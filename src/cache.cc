...
if((cache_type == IS_L1I || cache_type == IS_L1D) && reads_ready.size() == 0)
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            /*
               ...
               */
              //naman
            //   if (RQ.entry[index].instr_id == 457424 && cache_type  == IS_L1D)
                
            //   {
            //     cout << "current core cycle in handle read " << current_core_cycle[cpu] << " ste and way " << set << " " << way << endl; 
            //   }
            ...
            if (way >= 0) { // read hit
                ...
                if(RQ.entry[index].instr_id == DEBUG_INSTRUCTION && RQ.entry[index].instruction == INSTRUCTION) {
                    ...
                    cout << " physical_tag: " << (RQ.entry[index].full_physical_address >> LOG2_BLOCK_SIZE);
                ...
                if (RQ.entry[index].type == LOAD) {
                    ...
                    if (cache_type == IS_L1D) 
                        l1d_prefetcher_operate(RQ.entry[index].full_addr, RQ.entry[index].ip, 1, RQ.entry[index].type, RQ.entry[index].critical_ip_flag,RQ.entry[index].full_physical_address);	// RQ.entry[index].instr_id);  // changes made by naman ,RQ.entry[index].full_physical_address
                ...
                ACCESS[RQ.entry[index].type]++;
                //naman
                // cout << "address on hit " << hex << RQ.entry[index].full_addr <<endl;
                // cout << "address on hit physical " << hex << RQ.entry[index].full_physical_address <<endl;
                // cout << "address on hit " << hex <<RQ.entry[index].address << endl;
                ...
                reads_available_this_cycle--;
                    ...
                    if (cache_type == IS_L1D && RQ.entry[index].type == LOAD)
                    {
                        ...
                        for (int index_buffer =0 ; index_buffer<buffer_size ; index_buffer++){
                            ...
                            if((pf_buffer_l1[index_buffer].address == RQ.entry[index].full_physical_address>>LOG2_BLOCK_SIZE))
                                {   
                                    // cout << "Hit Milla" << endl; 
                                    get_hit_in_buffer++;
                                    // cout << index << endl;
                                    ...
                                    uint32_t set = get_set(pf_buffer_l1[index].address), way;
                                    way = (this ->*find_victim)(cpu, pf_buffer_l1[index].instr_id, set, block[set], pf_buffer[index].ip, pf_buffer[index].full_addr, pf_buffer[index].type);//have to fix fill_cpu to cpu
                                
                                if (block[set][way].valid == 0)
                                    block[set][way].valid = 1;
                                block[set][way].dirty = 0;
                                block[set][way].prefetch = (pf_buffer[index].type == PREFETCH || pf_buffer[index].type == PREFETCH_TRANSLATION || pf_buffer[index].type == TRANSLATION_FROM_L1D) ? 1 : 0;
                                block[set][way].used = 0;

                                //Neelu: Setting instruction and translation fields in L2C
                                

                                block[set][way].delta = pf_buffer_l1[index_buffer].delta;
                                block[set][way].depth = pf_buffer_l1[index_buffer].depth;
                                block[set][way].signature = pf_buffer_l1[index_buffer].signature;
                                block[set][way].confidence = pf_buffer_l1[index_buffer].confidence;
    
                                block[set][way].tag = pf_buffer_l1[index_buffer].address; //@Vishal: packet->address will be physical address for L1I, as it is only filled on a miss.
                                block[set][way].address = pf_buffer_l1[index_buffer].address;
                                block[set][way].full_addr = pf_buffer_l1[index_buffer].full_addr;
                                block[set][way].data = pf_buffer_l1[index_buffer].data;
                                block[set][way].cpu = pf_buffer_l1[index_buffer].cpu;
                                block[set][way].instr_id = pf_buffer_l1[index_buffer].instr_id;
                    ...
                    //naman nish
                                ...
                                //  if (cache_type == IS_L1D)
                                // {
                                //     cout << "MSHR INDEX "<< mshr_index << " merging with instr_id: " << MSHR.entry[mshr_index].instr_id <<endl;
                                // for(int i=0;i<ooo_cpu[0].L1D.MSHR.SIZE;i++)
                                //     cout << "address at index " << i << "is "<<hex<< MSHR.entry[i].full_addr << endl;
                                // }
                                //naman
                    ...
                    if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) { // this is a new miss
                        ...
                        else
                        {
                            ...
                            if(cache_type == IS_L1I || cache_type == IS_L1D) //@Vishal: VIPT
                            {
                                ...
                                //  if (RQ.entry[index].instr_id == 457424 && cache_type == IS_L1D)
                                // {
                                //     cout << "curren core cycle "<< current_core_cycle << endl;
                                // }
                                //naman
                        ...
                        if ((mshr_index == -1) && (MSHR.occupancy == MSHR_SIZE)) { // not enough MSHR resource
                            ...
                            if (RQ.entry[index].type == RFO) {
                                ...
                                else 
                                {
                                    ...
                                    if(RQ.entry[index].instr_id == DEBUG_INSTRUCTION && RQ.entry[index].instruction == INSTRUCTION) {
                                            ...
                                            for(int i=0; i<ooo_cpu[0].L1D.MSHR_SIZE;i++)
                                                cout << "instr id at mshr index: " << i << " is: " << ooo_cpu[0].L1D.MSHR.entry[i].instr_id << " address: " << hex << ooo_cpu[0].L1D.MSHR.entry[i].address  << " type: " << unsigned( ooo_cpu[0].L1D.MSHR.entry[i].type )<< endl;
                                            }
                                            cout << dec;
                            ...
                            if ((MSHR.entry[mshr_index].type == PREFETCH && RQ.entry[index].type != PREFETCH) ||(MSHR.entry[mshr_index].type == PREFETCH_TRANSLATION && RQ.entry[index].type != PREFETCH_TRANSLATION) || (MSHR.entry[mshr_index].type == TRANSLATION_FROM_L1D && RQ.entry[index].type != TRANSLATION_FROM_L1D )) {
                                ...
                                //naman Bug Fix problem was that when a new RQ request come and when it check for it in MSHR it sees entry of prefetcher 
                                //so it just replace all attribute of mshr with rq where cycle enqueued is zero which gives error of extreme high L1D Avg miss letency 
                        ...
                        if (miss_handled) {
                            ...
                            if (RQ.entry[index].type == LOAD) {
                                ...
                                if (cache_type == IS_L1D) 
                                    l1d_prefetcher_operate(RQ.entry[index].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, RQ.entry[index].critical_ip_flag,RQ.entry[index].full_physical_address);	//RQ.entry[index].instr_id);
                ...
                if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
                {
                    ...
                    if (way >= 0) { // prefetch hit
                        ...
                        if (PQ.entry[index].instr_id == DEBUG_INSTRUCTION && PQ.entry[index].instruction == INSTRUCTION) {
                                ...
                                cout << " fill_l1d: " << unsigned(PQ.entry[index].fill_l1d);
                                cout << " fill_l1i: " << unsigned(PQ.entry[index].fill_l1i);
                                cout << " origin level: " << PQ.entry[index].pf_origin_level<< endl;}
                        //naman nish
                        ...
                        if(PQ.entry[index].pf_origin_level < fill_level)
                        {
                            if (cache_type == IS_L1D)
                            {
                                ...
                                assert(0);
                                l1d_prefetcher_operate(PQ.entry[index].full_addr, PQ.entry[index].ip, 1, PREFETCH, PQ.entry[index].critical_ip_flag,RQ.entry[index].full_physical_address);	//, PQ.entry[index].prefetch_id);
                            }
                            else if ((cache_type == IS_L2C) && (RQ.entry[index].type != PREFETCH_TRANSLATION) && (RQ.entry[index].instruction == 0) && (RQ.entry[index].type != LOAD_TRANSLATION) && (RQ.entry[index].type != PREFETCH_TRANSLATION) && (RQ.entry[index].type != TRANSLATION_FROM_L1D))
                            {
                            ...
                            if (lower_level) {
                                if (cache_type == IS_LLC) {
                                    ...
                                    if (lower_level->get_occupancy(3, PQ.entry[index].address) == lower_level->get_size(3, PQ.entry[index].address))
                                        ...
                                        {
                                            if (cache_type == IS_L1D)
                                                l1d_prefetcher_operate(PQ.entry[index].full_addr, PQ.entry[index].ip, 0, PREFETCH, PQ.entry[index].critical_ip_flag,RQ.entry[index].full_physical_address);	// PQ.entry[index].prefetch_id); //naman ,RQ.entry[index].full_physical_address
                                            else if ((cache_type == IS_L2C) && (RQ.entry[index].type != PREFETCH_TRANSLATION) && (RQ.entry[index].instruction == 0) && (RQ.entry[index].type != LOAD_TRANSLATION) && (RQ.entry[index].type != PREFETCH_TRANSLATION) && (RQ.entry[index].type != TRANSLATION_FROM_L1D))
                                            {
        ...
        void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
        {
            ...
            //naman
            //implement filling of prefetch buffer here check cache type == L1D and packet type == prefetch
            //naman
#ifdef PF_BUFFER
            //naman
            if (cache_type == IS_L1D && packet->type == PREFETCH && packet->fill_level == FILL_L1 )
            {
        ...
        int CACHE::check_hit(PACKET *packet)
        {
            ...
            // // naman
            // if(NAME == "L1D"){
            //     // std::cout << "cache.cc ke ander ka address" << packet->full_virtual_address <<std::endl;
            //     // return 0;
            //     // std::cout << "NAME == L1D " <<std::endl;
            //     if(check_point == 0){
            //         previous_address = packet->full_virtual_address;
                    
            //         check_point++ ;
            //         // std::cout << "inside if condition of l1d in check hit " << check_point <<std::endl;
            //         return match_way;
            //     }
            //     // std::cout << "NAME == L1D after first if" <<std::endl;
            //     stride = (packet->full_virtual_address) - previous_address;
            ...
            //         return 0;
            //     }
            //     // std::cout << "NAME == L1D after both if" <<std::endl;
            // }
            // // naman
        ...
        int CACHE::add_rq(PACKET *packet)
        {
            ...
            if (index != -1) {
                ...
                else 
                {
                    ...
                    if (RQ.entry[index].instr_id ==  DEBUG_INSTRUCTION && RQ.entry[index].instruction == INSTRUCTION) {
                            ...
                            // if(RQ.entry[index].read_translation_merged[packet->request_from] == 1)
                            // cout << " read_translation_merged ";
                            // if(RQ.entry[index].write_translation_merged[packet->request_from] == 1)
                            // cout << " write_translation_merged ";
                            // if(RQ.entry[index].prefetch_translation_merged[packet->request_from] == 1)
                            // cout << " prefetch_translation_merged ";
            ...
            if(cache_type == IS_L1I || cache_type == IS_L1D)
            {
                ...
                //     if(cache_type == IS_L1D && packet->instr_id == 457424){
                //         // cout << "address" <<setw(16) <<  MSHR.entry[mshr_index].address << " event cycle" <<MSHR.entry[mshr_index].event_cycle << " cycle enqeued " << MSHR.entry[mshr_index].cycle_enqueued << " latency " << MSHR.entry[mshr_index].event_cycle - MSHR.entry[mshr_index].cycle_enqueued <<" " << " Instruction id "<<  MSHR.entry[mshr_index].instr_id<< " packet type"<<unsigned(MSHR.entry[mshr_index].type) <<endl;   
                //         cout << "current core cycle: " << dec << current_core_cycle[cpu] << endl;
                // }
    ...
    //naman nish
        ...
        int CACHE::add_pq(PACKET *packet)
        {
            ...
            //naman
            if(PQ.entry[index].cycle_enqueued == 0) 
                cout << "IN add_pq cycle enqueued is zero " << endl;
            //naman
        ...
        cout << " physical_addr: " << MSHR.entry[mshr_index].full_physical_address;
    ...
    if (packet->instr_id == DEBUG_INSTRUCTION && packet->instruction == INSTRUCTION && (queue->occupancy == queue->SIZE)) { 
                    ...
                    cout << " address: " << hex << packet->address;
                    cout << " full_addr: " << packet->full_addr << " check_addr: "<< check_address << dec;
                    cout << " cycle: " << current_core_cycle[packet->cpu] 
                    << " current_core_cycle: " << current_core_cycle[packet->cpu] << " event_cycle: " << packet->event_cycle << endl; }
            //naman nish
        ...
        void CACHE::add_nonfifo_queue(PACKET_QUEUE *queue, PACKET *packet)
        {
            ...
            //naman
            if (packet->cycle_enqueued == 0)
            {
                cout<<"cycle 0 "<< packet->instr_id<< endl;
            }
            ...
            if (packet->instr_id == DEBUG_INSTRUCTION && packet->instruction == INSTRUCTION) {
                            ...
                            // cout << " write_translation_merged ";
                            // else if(packet->prefetch_translation_merged[packet->request_from])
                            // cout << " prefetch_translation_merged ";
                            cout << " fill_level: " << queue->entry[index].fill_level;
                            cout << " index: " << index << " occupancy: " << queue->occupancy << endl; }
                    //naman nish
